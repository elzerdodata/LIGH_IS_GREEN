...
void DkVideoRenderer::rasterize_quick_menu() {
    ...
    auto rounded = [&](int x, int y, int w, int h, int radius,
                       uint32_t color) {
        if (w <= 0 || h <= 0) return;
        ...
        const int squared = r * r;
        for (int row = 0; row < r; ++row) {
            const int dy = r - row - 1;
            const int inset = std::clamp(
                r - static_cast<int>(std::sqrt(
                        std::max(0, squared - dy * dy))),
                0, r);
            fill(x + inset, y + row, w - 2 * inset, 1, color);
            fill(x + inset, y + h - row - 1, w - 2 * inset, 1, color);
        }
    };
    auto rounded_panel = [&](int x, int y, int w, int h, int radius,
                             uint32_t fillColor, uint32_t borderColor,
                             int thickness = 1) {
        rounded(x, y, w, h, radius, borderColor);
        rounded(x + thickness, y + thickness, w - 2 * thickness,
                h - 2 * thickness, std::max(0, radius - thickness),
                fillColor);
    };
    ...
    // Shared ZERODROID 1.0 visual language: navy depth, a thin lilac focus
    // border and teal reserved for a healthy live connection.
    const uint32_t panel = rgba(7, 15, 29, 248);
    const uint32_t surface = rgba(14, 25, 44, 244);
    const uint32_t edge = rgba(49, 65, 91, 255);
    const uint32_t accent = rgba(139, 92, 246, 255);
    const uint32_t active = rgba(73, 42, 155, 248);
    ...
    if (quick_state_.open) {
        ...
        rounded(menu.x - 3, menu.y - 3, menu.w + 6, menu.h + 6, 18,
                rgba(139, 92, 246, 24));
        rounded_panel(menu.x, menu.y, menu.w, menu.h, 17, panel, accent, 2);
        ...
        for (int row = 0; row < kQuickRowCount; ++row) {
            ...
            rounded_panel(rr.x, rr.y, rr.w, rr.h, 10, surface, edge, 1);
            blit_quick_text(labels[row], rr.x + 14, rr.y + 12);

            if (row == QuickPerformance) {
                rounded_panel(rr.x + 350, rr.y + 6, 104, rr.h - 12, 10,
                              quick_state_.performance ? active
                                                       : rgba(20, 34, 58, 244),
                              quick_state_.performance ? accent : edge, 1);
            ...
            if (row == QuickController) {
                rounded_panel(rr.x + 300, rr.y + 6, 154, rr.h - 12, 10,
                              quick_state_.xboxFaceLayout ? active
                                                          : rgba(20, 34, 58, 244),
                              quick_state_.xboxFaceLayout ? accent : edge, 1);
            ...
            if (row == QuickMouseSpeed || row == QuickResolution ||
                ...
                rounded_panel(minus.x, minus.y + 4, minus.w, minus.h - 8, 9,
                              rgba(20, 34, 58, 244), edge, 1);
                rounded_panel(plus.x, plus.y + 4, plus.w, plus.h - 8, 9,
                              rgba(20, 34, 58, 244), edge, 1);
        ...
        rounded_panel(warning_box.x, warning_box.y, warning_box.w,
                      warning_box.h, 13, rgba(31, 29, 66, 248), accent, 1);
        ...
        rounded_panel(reset.x, reset.y, reset.w, reset.h, 13,
                      surface, accent, 1);
        blit_quick_text("RESET IMAGE", reset.x + 55, reset.y + 10);

        QuickRect mouse_help = local(kQuickMouseHelpRect);
        rounded_panel(mouse_help.x, mouse_help.y, mouse_help.w,
                      mouse_help.h, 11, rgba(12, 23, 42, 244), edge, 1);
    ...
    if (quick_state_.sessionActionsOpen) {
        ...
        rounded(menu.x - 4, menu.y - 4, menu.w + 8, menu.h + 8, 22,
                rgba(139, 92, 246, 28));
        rounded_panel(menu.x, menu.y, menu.w, menu.h, 20, panel, accent, 2);
        blit_session_text("ZERODROID CONTROL CENTER", menu.x + 18,
                          menu.y + 16);

        const uint32_t connected = rgba(45, 212, 191, 255);
        ...
        rounded_panel(menu.x + menu.w - badgeW - 18, menu.y + 14,
                      badgeW, 34, 12,
                      healthy ? rgba(9, 60, 67, 244) : rgba(69, 49, 15, 244),
                      healthy ? connected : warning, 1);
        ...
        rounded_panel(status.x, status.y, status.w, status.h, 16,
                      rgba(12, 23, 42, 246), edge, 1);
        rounded_panel(actions.x, actions.y, actions.w, actions.h, 16,
                      rgba(12, 23, 42, 246), edge, 1);
        ...
        const auto action_button = [&](const QuickRect& absolute,
            ...
            rounded_panel(r.x, r.y, r.w, r.h, 11,
                          activeState ? active : surface, border,
                          activeState ? 2 : 1);
            fill(r.x + 10, r.y + 12, 3, r.h - 24, border);
        ...
        rounded_panel(help.x, help.y, help.w, help.h, 14,
                      rgba(31, 29, 66, 248), accent, 1);
        ...
        if (quick_state_.reconnectConfirmOpen) {
            ...
            rounded_panel(confirm.x, confirm.y, confirm.w, confirm.h, 18,
                          rgba(7, 15, 29, 252), warning, 2);
            ...
            rounded_panel(cancel.x, cancel.y, cancel.w, cancel.h, 12,
                          surface, edge, 1);
            rounded_panel(accept.x, accept.y, accept.w, accept.h, 12,
                          active, warning, 1);
